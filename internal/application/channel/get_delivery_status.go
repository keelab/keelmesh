...
func (s *Service) GetDeliveryStatus(_ context.Context, request *channelv1.GetDeliveryStatusRequest) (*channelv1.GetDeliveryStatusResponse, error) {
	...
	return &channelv1.GetDeliveryStatusResponse{MessageId: status.MessageID, ChannelId: status.ChannelID, State: string(status.State), AcceptedAtMs: status.AcceptedAt.UnixMilli(), UpdatedAtMs: status.UpdatedAt.UnixMilli(), Error: status.Error}, nil
}
