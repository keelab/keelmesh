...
func (c *Channel) RegisterHTTP(router *khttp.Router) error {
	...
	if err := channelhttp.Register(router, http.MethodPost, c.config.Path, "webhook-"+c.config.ID, http.HandlerFunc(c.serve)); err != nil {
		return err
	}
	return channelhttp.Register(router, http.MethodGet, c.config.Path+"/health", "webhook-health-"+c.config.ID, http.HandlerFunc(c.health))
}
